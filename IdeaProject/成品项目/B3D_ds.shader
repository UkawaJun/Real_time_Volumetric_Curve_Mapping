...
void vertex() {
	...
	do
	{
			...
			vec3 p2_d1 = (d1-p2);
			float csize = p.x/1.0;
			csize = (1.0-csize)*size1 + csize*size2;
			csize += (sin(p.x*360.0)+1.4)/80.0;
			vec3 sz =  normalize(cross(p2_d2,p2_d1));
			t1 += normalize(sz*p.z)*csize;
			
			vec3 sy = normalize(cross(sz,d2-d1));
			
			t1 += normalize(sy*p.y)*csize;
			
			NORMAL = normalize(t1- t0);
			result = t1;
			//result -= p0;
			rp = result;
	}while(false);
