...
void vertex() {
	...
	do
	{
			...
			//构建向量组： 
			vec3 triangle_a = cross((p1-p2),p3-p2);
			vec3 triangle_b = cross((p2-p3),p4-p3);
			vec3 d1_p2 = (d1+d3)/2.0;
			vec3 d3_p2 = (d3-d2);
			//vec3 d2_d3 = (p1-p2);
			//vec3 d2_d1 = (p4-p3);
			vec3 sz =  normalize(cross(d1_p2,d3_p2));
			sz = (1.0-pt.x)*triangle_a + triangle_b*pt.x;
			c1 += normalize(sz)*Lsize*p.z;
			
			vec3 sy = normalize(cross(sz,d3-d1));
			
			c1 += normalize(sy)*Lsize*p.y;
